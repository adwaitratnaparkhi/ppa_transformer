...
class ConllAttachment(object):
    ...
    def __init__(self, heads, label, prep, child, heads_next=None, heads_pos=None, heads_next_pos=None, sentence=None):
        self.heads = heads
        self.label = label
        self.prep = prep
        self.child = child
            ...
            for child_idx in children_indices:
                if sentence.is_valid_attachment(i, child_idx-1, max_head_distance, max_child_distance):
                    ...
                    if len(heads) > 1:  # don't take unambiguous cases
                        if get_heads_next:
                            attachment = EnglishAttachment(heads, gold_head_label, prep, child, heads_next=heads_next, heads_pos=heads_pos, heads_next_pos=heads_next_pos, sentence=sentence)
                        else:
                            attachment = EnglishAttachment(heads, gold_head_label, prep, child, heads_pos=heads_pos, heads_next_pos=heads_next_pos, sentence=sentence)
...
if ( len(sys.argv) != 5 ):
    sys.stderr.write("args: [Input TR dependency file] [Input TE dependency file] [Output TR prefix] [Output TE prefix]\n")
    sys.exit(1)

WSJ_FILE_TR=sys.argv[1]
